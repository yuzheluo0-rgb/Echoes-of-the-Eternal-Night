...
`#/battle` 是第一章战斗 demo（含新手引导）。和 `src/cards/**` 一样，**整块不 import `src/world/**`**。

- **第一章对应地图里第一片解锁的地貌**：草原 · 余烬营地（THE LAST HEARTH）。怪物图鉴要从
  地图的既有设定里长出来——草原的敌对动物是灰狼，隐秘据点是萤火树洞，营地的三个关卡是
  荒野巡夜 / 失落的商队 / 草原守望者。**不要自己另起一套设定**（早先误做成「灰烬墓园」，
  那是弃用原型里的东西，地图里没有）。
...
- 引擎是纯函数 + 确定性种子，30+ 项测试。`intentFor()` 会算出聚群等加成后的真实数值，
