...
export default memo(function WorldMinimap({position,selected,onChoose}:{position:string;selected:string;onChoose:(id:string)=>void}){
  ...
  return <aside className="world-minimap" aria-label="世界概览小地图"><div><span>山海图卷</span><small>点击定位</small></div><svg ref={svg} viewBox="-32 -23 64 46" onClick={choose} role="img" aria-label="当前位置与世界地貌"><rect x="-32" y="-23" width="64" height="46" fill="#17232b"/>{[...outlines].map(([biome,d])=><path key={biome} d={d} fill={PALETTE[biome]} opacity={biome==='ocean'?.32:1}/>)}{selected!==position&&<circle cx={destination.x} cy={destination.z} r="1.1" fill="none" stroke="#f3d08b" strokeWidth=".45"/>}<circle cx={actor.x} cy={actor.z} r="1.8" fill="#ffdf9b" opacity=".18"/><circle cx={actor.x} cy={actor.z} r=".85" fill="#ffe4a2" stroke="#151d22" strokeWidth=".35"/></svg><p><span />守夜人 <i />{TILES.length} 格 · 12 片风土</p></aside>;
});
