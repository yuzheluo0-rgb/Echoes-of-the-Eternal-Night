...
import { BookOpen, Gem, Droplets, Skull, Zap, ArrowLeft, ArrowRight, Check, ChevronRight, Compass, Crown, Flag, Flame, Footprints, HelpCircle, Home, LocateFixed, MapPin, Maximize, Minus, Mountain, Plus, RotateCcw, RotateCw, Snowflake, Sparkles, Sun, Trees, Volume2, VolumeX, Waves, Wind, X } from 'lucide-react';
import { footstep, setWorldAudio, setAudioVolume, sound } from '../audio';
import { advanceJourney, travelByBeacon, MAIN_SITES, BIOMES, DIRECTIONS, LANDMARKS, SAVE_KEY, TILE_MAP, TILES, findPath, parseSave, tileId, navigationTarget, SITE_SIZE, type Biome, type Landmark, type WorldSave } from './worldData';
...
export default function WorldMap() {
  ...
  const [panel, setPanel] = useState<'help' | 'atlas' | 'journal' | 'beacons' | null>(null), [site, setSite] = useState<Landmark | null>(null);
  ...
  const knownSites=LANDMARKS.filter(s=>s.kind!=='hidden'&&s.kind!=='event'||save.discovered.includes(s.id));
  ...
  const path = findPath(save.position, selected), steps = Math.max(path.length - 1, 0);
  ...
  const showSite = useCallback((id: string) => { const landmark = LANDMARKS.find(l => l.id === TILE_MAP.get(id)?.landmark); if (landmark) { setSite(landmark); setMissionIndex(0); sound('bell', audioRef.current); } }, []);
  const moveTo = useCallback((id: string) => {
    if (walkingRef.current) return;
    const path = findPath(saveRef.current.position, id);
  ...
  useEffect(() => {
    ...
    try {
      ...
      scene.current = engine; engine.setLabels(labels.current); engine.select(selectedRef.current, findPath(saveRef.current.position, selectedRef.current));
  ...
  useEffect(() => { scene.current?.setPaused(!!panel || !!site || !!error); }, [panel, site, error, attempt]);
  useEffect(() => { if (!walking) scene.current?.select(selected, findPath(save.position, selected)); }, [selected, save.position, walking, attempt]);
  ...
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      if (panel || site || walkingRef.current || error || !ready || event.ctrlKey || event.metaKey || event.altKey || (event.target as HTMLElement)?.closest('.world-clock') || /INPUT|TEXTAREA|SELECT/.test((event.target as HTMLElement)?.tagName || '')) return;
  ...
  }, [panel, site, error, ready, moveTo]);
  const chooseSite = (landmark: Landmark) => { setPanel(null); const id = tileId(landmark.q, landmark.r); if (!walkingRef.current) { setSelected(id); selectedRef.current = id; scene.current?.focus(id); sound('select', audioOn); } };
  const locateSite=(id:string)=>{const landmark=LANDMARKS.find(s=>s.id===id);if(landmark){setSite(null);chooseSite(landmark);}};
  ...
  const actionLabel = walking ? '正在行进' : !selectedTile.walkable ? selectedTile.biome === 'snow' ? '雪峰无法攀越' : '水域无法落脚' : steps ? '前往此处' : selectedSite ? '探索此地' : '已抵达此处';
  const siteList = (compact = false) => atlasSites.map((landmark, i) => { const SiteIcon = landmark.kind==='side'?Flag:landmark.kind==='event'?Sparkles:landmark.kind==='hidden'?Gem:ICONS[landmark.biome]; return <button key={landmark.id} className={`atlas-site ${selectedSite?.id === landmark.id ? 'active' : ''}`} onClick={() => chooseSite(landmark)} disabled={walking} style={{ '--biome': BIOMES[landmark.biome].color } as CSSProperties}>
    <span className="atlas-site-icon"><SiteIcon size={compact ? 15 : 18} /></span><span className="atlas-site-copy"><span>{landmark.name}</span><small>{landmark.id==='ocean'?'主线修复 · '+save.harbor.stage+'/3':BIOMES[landmark.biome].name}{!compact && ` · ${landmark.difficulty}`}</small></span><span className="atlas-site-state">{save.visited.includes(landmark.id) ? <Check size={13} /> : String(i + 1).padStart(2, '0')}</span>
  </button>; });
  return <main className={`world-page ${reduced ? 'world-reduced' : ''}`}>
    ...
    <div className={`world-labels ${labelsOn ? '' : 'labels-off'}`} aria-label="地图上的关卡地点">{knownSites.map(landmark => { const SiteIcon = ICONS[landmark.biome]; return <button key={landmark.id} ref={element => { if (element) labels.current.set(landmark.id, element); else labels.current.delete(landmark.id); }} className={`world-site-label site-${landmark.kind}`} style={{ '--biome': BIOMES[landmark.biome].color } as CSSProperties} onClick={() => chooseSite(landmark)} disabled={!ready || walking} aria-label={`定位${landmark.name}`}><span className="site-pin"><SiteIcon size={12} /></span><span className="site-label-name">{landmark.name}</span>{save.visited.includes(landmark.id) && <i />}</button>; })}</div>
    ...
    <section className="world-title"><p className="world-kicker"><span /> THE OVERWORLD <span /></p><h1>余烬之境</h1><p className="world-subtitle">长夜尚未结束，而旅途才刚刚开始。</p><div className="world-title-rule"><i /><span>主世界 · 自由探索</span><i /></div></section>
    <aside className={`world-atlas ${atlasOpen ? '' : 'atlas-collapsed'}`} aria-label="世界地貌与地点">
      ...
      <div className="atlas-content"><div className="atlas-progress"><span>探索印记</span><strong>{String(save.visited.length).padStart(2, '0')} <small>/ {LANDMARKS.length}</small></strong></div>{atlasTabs}<div className="atlas-sites">{siteList(true)}{!atlasSites.length&&<p className="atlas-empty">有些地方只在你靠近时显现。沿着六边格走走，留意路边的微光。</p>}</div><p className="atlas-note"><span />灯火所至，长夜退散。</p></div>
    </aside>
    <button className="mobile-atlas world-icon" aria-label="打开世界图志" onClick={() => setPanel('atlas')}><Compass size={19} /></button>
    <WorldMinimap position={save.position} selected={selected} onChoose={raw=>{if(walkingRef.current)return;const id=navigationTarget(raw);setSelected(id);selectedRef.current=id;scene.current?.focus(id);}}/>
    ...
    <div className="world-bottom">
      ...
      <section className="world-travel" aria-label="行走与地点信息">
        ...
        <div className="travel-destination" style={{ '--biome': BIOMES[selectedTile.biome].color } as CSSProperties}><span className="destination-icon"><Icon size={23} strokeWidth={1.3} /></span><div><p className="destination-eyebrow">{steps ? '下一站' : '当前选中'} <span>· {BIOMES[selectedTile.biome].name}</span></p><h3>{selectedSite?.name || `${BIOMES[selectedTile.biome].name}地块`}</h3><p className="destination-description">{selectedSite ? selectedSite.id==='ocean'?'9 格港口 · 修复 '+save.harbor.stage+'/3':selectedSite.kind==='side'?selectedSite.quest!.npc:selectedSite.kind==='event'?'一个等待回应的故事':selectedSite.kind==='main'?SITE_SIZE[selectedSite.id]+' 格建筑群 · 前庭入口 · 3 个关卡':selectedSite.difficulty+' · 3 个可选关卡' : BIOMES[selectedTile.biome].description}</p></div></div>
        <div className="travel-action"><span className="travel-distance"><Footprints size={13} />{walking ? '循光而行…' : steps ? `${steps} 格 · 约 ${Math.ceil(steps * .48)} 秒` : selectedTile.walkable ? '脚下的世界，仍有回响' : selectedTile.biome === 'snow' ? '沿隘口穿越雪山' : '沿栈桥探索海域'}</span><button className="world-primary" aria-label={actionLabel} onClick={() => moveTo(selected)} disabled={!ready || !!error || walking || !selectedTile.walkable || (!steps && !selectedSite)}>{walking ? <span className="walking-dots" aria-hidden="true">···</span> : <Footprints size={16} />}<span>{actionLabel}</span><ArrowRight size={17} /></button></div>
      </section>
      <footer className="world-footer"><span><span className="status-dot" /> {save.mission ? `已选关卡 · ${save.mission}` : '自由探索 · 进度自动保存'}</span><span>永夜历 · 第七纪元 <b>✧</b> WORLD 01</span></footer>
    </div>
