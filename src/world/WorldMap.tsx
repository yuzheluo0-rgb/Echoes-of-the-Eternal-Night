...
import { Lock, BookOpen, Gem, Droplets, Skull, Zap, ArrowLeft, ArrowRight, Check, ChevronRight, Compass, Crown, Flag, Flame, Footprints, HelpCircle, Home, LocateFixed, MapPin, Maximize, Minus, Mountain, Plus, RotateCcw, RotateCw, Snowflake, Sparkles, Sun, Trees, Waves, Wind, X } from 'lucide-react';
import { footstep, setWorldAudio, setWorldTrack, setAudioVolume, sound } from '../audio';
import { parseScore, TRACK_KEY } from './worldScores';
import WorldScoreMenu from './WorldScoreMenu';
...
export default function WorldMap() {
  ...
  const [track, setTrack] = useState(() => { try { return parseScore(localStorage.getItem(TRACK_KEY)); } catch { return parseScore(null); } }), trackRef = useRef(track);
  ...
  saveRef.current = save; selectedRef.current = selected; walkingRef.current = walking; audioRef.current = audioOn; trackRef.current = track;
  ...
  useEffect(() => { setAudioVolume(.36);const update=()=>setWorldAudio(audioOn&&!document.hidden,TILE_MAP.get(saveRef.current.position)!.biome,trackRef.current);update();document.addEventListener('visibilitychange',update);return()=>{document.removeEventListener('visibilitychange',update);setWorldAudio(false);};},[audioOn]);
  useEffect(()=>{if(audioOn&&!document.hidden)setWorldAudio(true,currentTile.biome,trackRef.current);},[currentTile.biome,audioOn]);
  // Swapping tracks fades the notes across on the running soundscape, so the biome beds never restart.
  useEffect(()=>{try{localStorage.setItem(TRACK_KEY,track);}catch{/* The current choice still applies without storage. */}setWorldTrack(track);},[track]);
  useEffect(() => {
  ...
  return <main className={`world-page ${reduced ? 'world-reduced' : ''}`}>
    ...
    <header className="world-header">
      ...
      <div className="world-wallet" aria-label={`烬火 ${save.embers}，回声 ${save.echoes}`}><span><Flame size={13}/>{save.embers}</span><span><Sparkles size={13}/>{save.echoes}</span></div><nav className="world-top-actions" aria-label="地图设置"><button className="world-icon" aria-label="旅途手记" onClick={()=>setPanel('journal')}><BookOpen size={18}/></button><button className="world-icon" aria-label="信标传送" disabled={walking} onClick={()=>setPanel('beacons')}><Zap size={18}/></button><WorldScoreMenu on={audioOn} track={track} onToggle={() => { sound('select', !audioOn); setAudioOn(!audioOn); }} onPick={id => { sound('select', audioOn); setTrack(id); }} /><button className="world-icon" aria-label="地图操作说明" onClick={() => setPanel('help')}><HelpCircle size={18} /></button><a className="world-icon" href="#/opening" aria-label="返回序幕"><Home size={17} /></a></nav>
    </header>
